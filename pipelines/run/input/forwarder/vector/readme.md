...
A single receive wrapper covers all three modes — selected by receiver flags:
...
- **receive** (default) — Filter and drop events based on rules; surviving events are returned to Vector to ship.
- **optimize** — `receiverOptimize: true` — additionally encode emitted events for 50–80% volume reduction.

## Configuration Files

- `receive/config.yaml` — single wrapper used for all three modes (mode is selected by `receiverReadOnly` / `receiverOptimize` flags)
...
2. Log10x reads, runs the receiver pipeline (report / receive / optimize), and — unless `receiverReadOnly` is set — writes the resulting events back via the Fluent Forward protocol to `/tmp/tenx-vector-out.sock`
