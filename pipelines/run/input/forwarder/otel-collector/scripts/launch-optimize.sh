#!/bin/bash
# Launch script for OpenTelemetry Collector + Log10x Optimizer (Linux)
#
# This script starts both Log10x optimizer and OpenTelemetry Collector
# in the correct order for event optimization and transformation.
...
OTEL_CONFIG="${TENX_MODULES}/pipelines/run/modules/input/forwarder/otel-collector/optimize/tenxNix.yaml"
...
echo -e "${GREEN}========================================${NC}"
echo -e "${GREEN}OpenTelemetry Collector + Log10x Optimizer${NC}"
echo -e "${GREEN}========================================${NC}"
echo ""
...
# Start Log10x optimizer
echo -e "${GREEN}Starting Log10x optimizer...${NC}"
$TENX_BIN @run/input/forwarder/otel-collector/optimize @apps/edge/optimizer > "$LOG_DIR/tenx-optimizer.log" 2>&1 &
TENX_PID=$!
echo "Log10x PID: $TENX_PID"
echo "Log file: $LOG_DIR/tenx-optimizer.log"
...
for i in {1..30}; do
    ...
    if [ $i -eq 30 ]; then
        ...
        echo "Check log file: $LOG_DIR/tenx-optimizer.log"
        cleanup
    fi
    sleep 1
done
...
echo -e "${GREEN}========================================${NC}"
echo ""
echo "Log10x Optimizer:          PID $TENX_PID"
echo "OpenTelemetry Collector:   PID $OTELCOL_PID"
echo ""
echo "Event Flow:"
echo "  OTel Collector → TCP (4318) → Log10x → TCP (4319) → OTel Collector → Splunk/Elastic"
echo ""
echo "Logs:"
echo "  Log10x:    $LOG_DIR/tenx-optimizer.log"
